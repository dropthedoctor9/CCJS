...
// in  modern times, to write strings we use backticks, and by using backticks we have string interpolation which gives us placeholders and we can inject variables(strings, numbers, etc) where-ever we can. we can also use methods here which is an advantage

console.log(`Hello my name is ${firstName[0]} and my repo count is ${repoCount}`)// we can access key value pair here also 

const gameName= new String("poulentspacegiggle") // this is another way of declaring strings, in this case we use a new keyword and hence we use objects in JavaScript and inside the construtor we write its name/ initialise
...
console.log(gameName.__proto__) // this is just a syntax to access prototype, here the result will come empty object/method but in console we will get all methods
